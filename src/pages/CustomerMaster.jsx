...
export default function CustomerMaster() {
  ...
  const [visibleCols, setVisibleCols] = useState(() => {
    const saved = localStorage.getItem('customer-visible-cols');
    if (saved) { try { return JSON.parse(saved); } catch {} }
    return { id: true, name: true, email: true, phone: true, gst: true, action: true };
  });

  useEffect(() => {
    localStorage.setItem('customer-visible-cols', JSON.stringify(visibleCols));
  }, [visibleCols]);
  ...
  const load = async (signal) => {
    try {
      setLoading(true);
      const res = await api.get("/customers", { params: table, signal });
  ...
  const save = async () => {
    ...
    try {
      setSaving(true);
      if (editRow) {
        await api.put(`/customers/${editRow.id}`, form);
      } else {
        await api.post("/customers", form);
      }
      setShow(false);
      setReloadKey(k => k + 1);
      toast.success(editRow ? "Customer updated" : "Customer created");
    } catch (e) {
  ...
  const columns = useMemo(() => {
    const all = [
      ...
      visibleCols.name && { name: "Customer Name", selector: r => r.name, sortable: true, sortField: "name" },
    ...
    <div className="card card-outline card-primary">
      <div className="card-header">
        <div className="row g-2 align-items-center">
          <div className="col-12 col-md-6"><h3 className="card-title">Customer Master</h3></div>
          <div className="col-12 col-md-6">
            <div className="d-flex gap-2 flex-wrap justify-content-md-end">
              <input className="form-control form-control-sm" style={{ maxWidth: 220 }}
                placeholder="Search customers..." value={searchInput} onChange={e => setSearchInput(e.target.value)} />
              <button className="btn btn-primary btn-sm" onClick={openAdd}><i className="fas fa-plus"></i> Add Customer</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <Modal show={show} onHide={() => setShow(false)} backdrop="static" size="lg">
        <Modal.Header closeButton><Modal.Title>{editRow ? "Edit Customer" : "Add Customer"}</Modal.Title></Modal.Header>
        <Modal.Body>
          <Row>
