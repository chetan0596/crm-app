...
export default function LeadStages() {
  ...
  const load = async (signal) => {
    ...
    try {
      setLoading(true);
      const r = await api.get("/lead-stages", {
        params: {
          page: table.page,
  ...
  const moveRow = async (row, dir) => {
    ...
    try {
      ...
      await api.post('/lead-stages/reorder', {
  ...
  const openAdd = () => {
    setEdit(null);
    setForm({ name: "", color: "#0ea5e9", priority: "", status: true });
    setShow(true);
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      if (edit?.id) {
        await api.put(`/lead-stages/${edit.id}`, payload);
        toast.success("Lead Stage updated");
      } else {
        await api.post(`/lead-stages`, payload);
  ...
  const remove = async (row) => {
    ...
    try {
      await api.delete(`/lead-stages/${row.id}`);
  ...
  return (
    <div className="p-4">
      <div className="mb-4 d-flex align-items-center justify-content-between flex-wrap gap-2">
        <div>
          <h4 className="fw-semibold mb-1" style={{ color: "#1e293b" }}>Lead Stages</h4>
          <div className="text-secondary small">Set priority and pick a color</div>
        </div>
        {canCreate("leads") && (
...
persistTableHead
      ...
      <Modal show={show} onHide={() => setShow(false)} centered backdrop="static">
        <Modal.Header closeButton>
          <Modal.Title>{edit ? "Edit" : "Create"} Lead Stage</Modal.Title>
        </Modal.Header>
        <Modal.Body>
              ...
              {COLOR_OPTIONS.map((c) => (
                <button
                  ...
                  style={{
                    ...
                    background: c,
                    position: "relative",
                    boxShadow: form.color === c ? "0 0 0 3px rgba(37,99,235,0.35)" : "none",
                  }}
                >
                  {form.color === c ? (
                    <i className="fas fa-check" style={{ color: "white", fontSize: 16 }}></i>
                  ) : null}
                </button>
              ))}
            </div>
