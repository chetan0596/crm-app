...
export default function Sales() {
  ...
  const [form, setForm] = useState({
    ...
    invoice_number: "", notes: "", discount: 0, discount_percentage: 0, tax: 0, tax_percentage: 0, tax_name: "", addless_amount: 0, addless_title: "Addless", items: []
  });
  const [saving, setSaving] = useState(false);
  const [editingId, setEditingId] = useState(null);

  const [visibleCols, setVisibleCols] = useState(() => {
    const saved = localStorage.getItem('sales-visible-cols');
    if (saved) { try { return JSON.parse(saved); } catch {} }
    return { id: true, invoice_type: true, invoice_number: true, customer: true, warehouse: true, date: true, total: true, action: true };
  });

  useEffect(() => {
    localStorage.setItem('sales-visible-cols', JSON.stringify(visibleCols));
  }, [visibleCols]);
  ...
  useEffect(() => {
    if (form.invoice_type === 'Tax Invoice') {
      let totalTax = 0;
      form.items.forEach(it => {
        if (it.item_id && it.tax_id) {
          const taxInfo = getItemTaxInfo(it.item_id, it.tax_id, it.quantity, it.price, it.discount);
          totalTax += taxInfo.taxAmount;
        }
      });
      setForm(f => ({ ...f, tax: totalTax.toFixed(2) }));
    }
  }, [form.tax_type, form.gst_type, form.invoice_type]);

  // Generate next invoice number
  const generateInvoiceNumber = () => {
    if (!rows || rows.length === 0) return 'INV001';
  ...
  const openEdit = useCallback((row) => {
    ...
    setForm({
      ...
      invoice_date: row.invoice_date ? row.invoice_date.split('T')[0] : new Date().toISOString().split('T')[0],
    ...
    setForm(f => {
      ...
      if (field === 'item_id') {
        ...
        if (selectedItem) {
          ...
          formItems[idx].tax_id = selectedItem.tax_id || null;
        }
      }
      return { ...f, items: formItems };
    });
    
    // Recalculate tax from items when item, quantity, price, tax_id, or discount changes
    if (field === 'item_id' || field === 'quantity' || field === 'price' || field === 'tax_id' || field === 'discount') {
      setTimeout(() => {
        ...
        if (form.invoice_type === 'Tax Invoice') {
          ...
          totalTaxPercentage = taxableAmount > 0 ? (totalTax / taxableAmount) * 100 : 0;
        }
        
        setForm(f => ({ 
          ...f, 
          tax: totalTax.toFixed(2), 
          tax_percentage: totalTaxPercentage.toFixed(2)
        }));
      }, 100);
    }
  };
  ...
  const getItemTaxInfo = (itemId, taxId, quantity, price, discount = 0) => {
    if (!itemId || form.invoice_type !== 'Tax Invoice') {
      return { taxAmount: 0, taxPercentage: 0, taxName: '', baseAmount: 0 };
    }
    
    // Use the selected tax_id if available, otherwise fall back to item's default tax
    ...
    if (!itemTax) return { taxAmount: 0, taxPercentage: 0, taxName: '', baseAmount: 0 };
  ...
  const calculateTotals = () => {
    ...
    form.items.forEach(it => {
      ...
      // For Tax Inclusive, use baseAmount as the net amount
      // For Tax Exclusive, use itemTotal (which includes tax in the calculation later)
  ...
  const handleDiscountChange = (field, value) => {
    ...
    // Recalculate tax after discount change
    setTimeout(() => {
      const discount = parseFloat(field === 'discount' ? value : form.discount) || 0;
      ...
      if (form.invoice_type === 'Tax Invoice') {
        form.items.forEach(it => {
          if (it.item_id && it.tax_id) {
            const itemTax = taxes.find(t => String(t.id) === String(it.tax_id));
            if (itemTax) {
              const itemAmount = (parseFloat(it.quantity) || 0) * (parseFloat(it.price) || 0);
              const itemTaxAmount = (itemAmount * itemTax.percentage) / 100;
              totalTax += itemTaxAmount;
            }
          }
  ...
  const handleTaxChange = (field, value) => {
    ...
    if (field === 'tax') {
      ...
      const percentage = taxableAmount > 0 ? (taxAmount / taxableAmount) * 100 : 0;
      setForm(f => ({ ...f, tax: taxAmount, tax_percentage: percentage.toFixed(2) }));
    } else if (field === 'tax_percentage') {
      const percentage = parseFloat(value) || 0;
      const amount = (percentage / 100) * taxableAmount;
      setForm(f => ({ ...f, tax_percentage: percentage, tax: amount.toFixed(2) }));
    } else if (field === 'tax_name') {
      const selectedTax = taxes.find(t => t.name === value);
      if (selectedTax) {
        const amount = (selectedTax.percentage / 100) * taxableAmount;
        setForm(f => ({ ...f, tax_name: value, tax_percentage: selectedTax.percentage, tax: amount.toFixed(2) }));
      } else {
        setForm(f => ({ ...f, tax_name: value, tax_percentage: 0, tax: 0 }));
      }
    }
  };
  ...
  const save = async () => {
    ...
    if (!form.invoice_number) { toast.warning("Invoice number required"); return; }
  ...
  const columns = useMemo(() => {
    const all = [
      ...
      visibleCols.invoice_type && { name: "Type", selector: r => r.invoice_type, sortable: true, sortField: "invoice_type", width: "100px" },
      visibleCols.invoice_number && { name: "Invoice #", selector: r => r.invoice_number, sortable: true, sortField: "invoice_number" },
      visibleCols.customer && { name: "Customer", selector: r => r.customer?.name, sortable: true, sortField: "customer_id" },
      visibleCols.warehouse && { name: "Godown", selector: r => r.warehouse?.name || '-', sortable: true, sortField: "warehouse_id", width: "140px" },
      visibleCols.date && { name: "Date", selector: r => r.invoice_date, sortable: true, sortField: "invoice_date", width: "110px" },
      ...
      visibleCols.action && {
        ...
        cell: row => (
          <div className="btn-group btn-group-sm">
            <button className="btn btn-outline-info" onClick={() => viewSale(row)}><i className="fas fa-eye"></i></button>
    ...
    <div className="card card-outline card-primary">
      <div className="card-header">
        <div className="row g-2 align-items-center">
          <div className="col-12 col-md-6"><h3 className="card-title">Sales Invoices</h3></div>
          <div className="col-12 col-md-6">
            <div className="d-flex gap-2 flex-wrap justify-content-md-end">
              <input className="form-control form-control-sm" style={{ maxWidth: 220 }}
                placeholder="Search sales..." value={searchInput} onChange={e => setSearchInput(e.target.value)} />
              <button className="btn btn-primary btn-sm" onClick={openAdd}><i className="fas fa-plus"></i> New Sale</button>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="d-flex align-items-center gap-2 flex-wrap">
          ...
          <Dropdown>
            <Dropdown.Toggle variant="outline-secondary" size="sm">
              ...
              {[visibleCols.id && 'ID', visibleCols.invoice_type && 'Type', visibleCols.invoice_number && 'Invoice', visibleCols.customer && 'Customer', visibleCols.warehouse && 'Godown', visibleCols.date && 'Date', visibleCols.total && 'Total'].filter(Boolean).join(', ') || 'None'}
            </Dropdown.Toggle>
            <Dropdown.Menu>
              {['id','invoice_type','invoice_number','customer','warehouse','date','total'].map(col => (
                <Dropdown.Item key={col} active={visibleCols[col]} onClick={() => setVisibleCols(v => ({ ...v, [col]: !v[col] }))}>
                  ...
                  {col === 'invoice_number' ? 'Invoice #' : col === 'warehouse' ? 'Godown' : col === 'id' ? 'ID' : col.charAt(0).toUpperCase() + col.slice(1)}
                </Dropdown.Item>
              ))}
            </Dropdown.Menu>
          </Dropdown>
      ...
      <Modal show={show} onHide={() => setShow(false)} backdrop="static" size="xl">
        <Modal.Header closeButton><Modal.Title>{editingId ? 'Edit Sale' : 'New Sale'}</Modal.Title></Modal.Header>
        <Modal.Body>
          ...
          <Row className="mb-3">
            ...
            <Col md={6}>
              <div className="bg-light p-3 rounded border h-100">
                ...
                <Form.Group className="mb-3">
                  <Form.Label className="fw-bold small">Customer Name <span className="text-danger">*</span></Form.Label>
                  <Form.Select size="sm" value={form.customer_id} onChange={e => setForm(f => ({ ...f, customer_id: e.target.value }))}>
            ...
            <Col md={6}>
              <div className="bg-light p-3 rounded border h-100">
                <h6 className="text-primary mb-3 border-bottom pb-2">
                  <i className="fas fa-file-invoice me-2"></i>Invoice Info
                </h6>
                <Row>
                  <Col md={6}>
                    <Form.Group className="mb-2">
                      <Form.Label className="fw-bold small">Invoice # <span className="text-danger">*</span></Form.Label>
                      <Form.Control size="sm" value={form.invoice_number} onChange={e => setForm(f => ({ ...f, invoice_number: e.target.value }))} placeholder="e.g., INV-001" />
                    </Form.Group>
                  </Col>
                  <Col md={6}>
                    <Form.Group className="mb-2">
                      <Form.Label className="fw-bold small">Invoice Date <span className="text-danger">*</span></Form.Label>
                      <Form.Control type="date" size="sm" value={form.invoice_date} onChange={e => setForm(f => ({ ...f, invoice_date: e.target.value }))} />
                    </Form.Group>
                  </Col>
                </Row>
                <Row>
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={save} disabled={saving}>
            {saving && <span className="spinner-border spinner-border-sm me-2"></span>} {editingId ? 'Update Sale' : 'Save Sale'}
          </Button>
        </Modal.Footer>
      </Modal>
      ...
      <Modal show={viewShow} onHide={() => setViewShow(false)} size="lg">
        <Modal.Header closeButton><Modal.Title>Sale Details</Modal.Title></Modal.Header>
        <Modal.Body>
          {viewData && (
            <>
              {/* Header Info */}
              <div className="bg-light p-3 rounded border mb-3">
                <Row className="mb-2">
                  <Col md={3}><strong className="text-muted small">Customer:</strong><br/>{viewData.customer?.name}</Col>
                  <Col md={3}><strong className="text-muted small">Invoice #:</strong><br/>{viewData.invoice_number}</Col>
                  <Col md={3}><strong className="text-muted small">Type:</strong><br/>{viewData.invoice_type}</Col>
                  <Col md={3}><strong className="text-muted small">Date:</strong><br/>{viewData.invoice_date ? new Date(viewData.invoice_date).toLocaleDateString('en-IN') : '-'}</Col>
                </Row>
                <Row className="mb-2">
        ...
        <Modal.Footer>
          <Button
            ...
            onClick={() => {
              ...
              setViewShow(false);
              nav(`/credit-notes?saleId=${viewData.id}`);
            }}
          >
            Create Credit Note
          </Button>
