...
export default function CreditNotesReport() {
  const canViewReport = canView("credit-notes-report");
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const res = await api.get("/reports/credit-notes", { params });
  ...
  const handleDownloadCSV = () => {
    ...
    setCsvLoading(true);
    setTimeout(() => {
      downloadCsv("credit-notes-report.csv", data);
      setCsvLoading(false);
    }, 100);
  };
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      const res = await api.get("/reports/credit-notes/pdf", { params, responseType: "blob" });
      const blobUrl = window.URL.createObjectURL(new Blob([res.data], { type: "application/pdf" }));
      const link = document.createElement("a");
      link.href = blobUrl;
      link.download = "credit-notes-report.pdf";
      document.body.appendChild(link);
      link.click();
      link.remove();
  ...
  return (
    <div className="p-4">
      <div className="mb-4">
        <h4 className="fw-semibold mb-1" style={{ color: "#1e293b" }}>Credit Notes Report</h4>
        <div className="text-secondary small">Customer returns/discount adjustments summary</div>
      </div>
      ...
      <Card className="border" style={{ borderColor: "#e2e8f0" }}>
        <Card.Header className="bg-white d-flex justify-content-between align-items-center">
          <div className="fw-semibold">Credit Notes</div>
            ...
            <Table hover className="mb-0">
              ...
              <tbody>
                {loading ? (
                  ...
                  data.map((r) => (
                    <tr key={r.id}>
                      <td>{r.credit_note_date}</td>
                      <td className="fw-semibold">{r.credit_note_number}</td>
